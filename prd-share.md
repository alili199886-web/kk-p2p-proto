...
> 版本 V1.3 · 2026-08 · 产品：Ali
> 配套原型：C 端 H5 v6.6（文中截图均取自该原型；分享层内含「演示容器」切换器，仅用于演示，不上线）
...
体育侧**无原生 App，只有 H5**，会运行在四类容器中。分享能力取决于宿主是否注入 IM SDK（`im2-link-jssdk`）。

| 图1 金刚 App 内 | 图2 现金盘内嵌 | 图3 PC 浏览器 |
|---|---|---|
| ![金刚内](./img/s1_kk.png) | ![现金盘](./img/s2_cash.png) | ![PC](./img/s3_pc.png) |
...
- **复制链接**：所有容器常驻，是最终兜底；PC 浏览器额外提供二维码，便于手机扫码打开；
...
| 场景 | 触发条件 | 处理 |
|---|---|---|
| 订单已失效 | 已售罄 / 已封盘 / 已关闭 / 已结算 / 赛事已开赛（共 5 种） | webview 展示对应失效状态与说明，主按钮改为「前往查看其他挂单」；落地时弹失效提示并引导浏览其他挂单 |
| 订单不存在 | 订单ID 非法或数据已删除 | 展示「该挂单已不存在」，落地按失效处理（兜底，非 5 种失效状态之一） |
| 查看时有效、落地时失效 | 展示后至点击期间被买走或被封盘 | 以落地时实时校验为准，按失效流程提示，不进入购物车 |
| 商户标识缺失或非法 | `m` 参数丢失、被篡改或商户已下线 | 回落到金刚体育默认入口，其余流程不变 |
| 非金刚容器点击分享到金刚 | 理论上按钮已隐藏，异常情况下被触发 | 走 2.2 降级弹窗：复制链接 / 下载金刚通讯 / 取消 |
| 未安装 TG | 点击分享到 TG | 打开 TG 网页版，由 TG 自带下载引导，不阻断流程 |
| PC 端分享 | 在 PC 浏览器点击分享 | 提供复制链接与二维码，不尝试唤起手机应用 |
...
| 现金盘容器内按钮 | Given 体育 H5 内嵌在现金盘站点；When 打开分享层；Then 不展示「分享到金刚」，仅展示「分享到 TG」与复制链接 |
| 手机浏览器按钮 | Given 手机浏览器直接打开体育 H5；When 打开分享层；Then 不展示「分享到金刚」，仅展示「分享到 TG」与复制链接 |
| PC 端按钮 | Given PC 浏览器打开体育 H5；When 打开分享层；Then 展示「分享到 TG」、复制链接与二维码入口 |
...
| 失效单落地 | Given 挂单已售罄；When 接收方点击 webview；Then 不弹购物车，弹「订单已失效」提示并说明原因，提供「查看其他 P2P 挂单」 |
