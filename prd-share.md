...
> 版本 V1.6 · 2026-09 · 产品：Ali
> 配套原型：C 端 H5 v7.4（文中截图均取自该原型；分享层内含「演示容器」切换器与「模拟官方赔率反超」按钮，仅用于演示，不上线）
...
点击 webview → 唤起体育 → 解析 orderId 与 m
  → 按 m 打开对应平台的体育入口
  → 服务端校验挂单状态
      ├── 赔率变化导致挂单下架（仍在卖出中，但卖出赔率已低于官方赔率）
      │     → 回到赛事页，不打开购物车、不选中投注项
      │     → 弹窗提示「该 P2P 挂单已下架」
      ├── 有效（卖出中，含部分成交；有可卖额度；赛事未开赛；赔率不低于官方）
      │     → 进入该挂单所属赛事的比赛详情
      │     → 自动选中该投注项
...
**场景**：分享发出后官方赔率上涨并超过该挂单的卖出赔率。挂单本身仍是「卖出中」，但卖单簿只展示不低于官方赔率的档位，该档位随之从卖单簿下架——接收方点进来会找不到这笔挂单，且没有任何反馈。

**这不属于订单失效**：挂单未售罄、未封盘、未关闭、未结算，官方赔率回落后该档位会自动恢复，因此不走「订单已失效」的提示。

**处理规则**：

- 落地时在既有 5 种失效判定之外，追加一次判定：挂单卖出赔率 < 当前官方赔率；
- 命中则**先回到赛事页**（关闭分享页、不打开购物车、不选中任何投注项），再弹出提示弹窗；
- 挂单赔率等于官方赔率时视为可购买，不触发该提示；
- 分享内容页在此状态下追加一行提示，让接收方在点击前就有预期。

| 位置 | 文案 |
|---|---|
| 弹窗标题 | 该 P2P 挂单已下架 |
| 弹窗正文 | 因赔率变化，该挂单已下架，请选择其他 P2P 订单进行购买。 |
| 弹窗按钮 | 我知道了（单按钮，点击关闭弹窗，用户停留在赛事列表） |
| 内容页提示行 | 当前官方赔率 @{官方赔率} 已高于本挂单赔率，该挂单已从卖单簿下架 |
...
| 赔率变化导致下架 | 分享后官方赔率上涨并超过挂单赔率 | 不算失效。回到赛事页后弹「该 P2P 挂单已下架」单按钮提示，不打开购物车；内容页提前给出提示行 |
...
| 赔率下架提示 | Given 挂单赔率 @1.05、官方赔率已涨到 @1.12；When 接收方点击 webview 落地；Then 回到赛事页，不打开购物车、不选中投注项，弹出「该 P2P 挂单已下架」单按钮提示 |
| 赔率相等不提示 | Given 挂单赔率与官方赔率相等；When 落地；Then 正常进入购物车，不弹该提示 |
